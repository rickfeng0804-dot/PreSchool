...
export const Header: React.FC<HeaderProps> = ({ onLoginClick, onHomeClick, onSettingsClick, isAdmin, onLogout }) => {
  return (
          ...
          {isAdmin ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold p-3 rounded-2xl border-b-4 border-gray-300 active:border-b-0 active:translate-y-1 transition-all"
                title="系統設定"
              >
                <Settings size={20} />
              </button>
              <button
                onClick={onLogout}
                className="bg-red-400 hover:bg-red-500 text-white font-bold py-2 px-4 md:px-6 rounded-2xl border-b-4 border-red-600 active:border-b-0 active:translate-y-1 transition-all flex items-center gap-2"
              >
                <LogOut size={18} />
